export default {
    // Common
    loading: 'Laden...',
    error: 'Fehler',
    success: 'Erfolg',
    cancel: 'Abbrechen',
    save: 'Speichern',
    delete: 'Löschen',
    edit: 'Bearbeiten',
    add: 'Hinzufügen',
    back: 'Zurück',
    next: 'Weiter',
    previous: 'Zurück',
    close: 'Schließen',
    confirm: 'Bestätigen',
    yes: 'Ja',
    no: 'Nein',
    all: 'Alle',
    view: 'Anzeigen',
    search: 'Suchen',
    filter: 'Filter',
    sort: 'Sortieren',
    refresh: 'Aktualisieren',
    retry: 'Wiederholen',
    reload: 'Neu laden',

    // Navigation
    menu: 'Menü',
    cart: 'Warenkorb',
    login: 'Anmelden',
    logout: 'Abmelden',
    profile: 'Profil',
    dashboard: 'Dashboard',
    settings: 'Einstellungen',
    userMenu: 'Benutzermenü',
    user: 'Benutzer',
    editProfile: 'Profil Bearbeiten',
    save: 'Speichern',
    cancel: 'Abbrechen',
    personalInformation: 'Persönliche Informationen',
    accountInformation: 'Kontoinformationen',
    accountSettings: 'Kontoeinstellungen',
    fullName: 'Vollständiger Name',
    enterFullName: 'Geben Sie Ihren vollständigen Namen ein',
    enterUsername: 'Geben Sie Ihren Benutzernamen ein',
    enterEmail: 'Geben Sie Ihre E-Mail-Adresse ein',
    notProvided: 'Nicht angegeben',
    memberSince: 'Mitglied Seit',
    lastLogin: 'Letzter Login',
    notAvailable: 'Nicht verfügbar',
    changePassword: 'Passwort Ändern',
    changePasswordDescription: 'Aktualisieren Sie Ihr Kontopasswort für Sicherheit',
    notificationSettings: 'Benachrichtigungseinstellungen',
    notificationSettingsDescription: 'Konfigurieren Sie, wie Sie Benachrichtigungen erhalten',
    privacySettings: 'Datenschutzeinstellungen',
    privacySettingsDescription: 'Verwalten Sie Ihre Datenschutzpräferenzen',
    change: 'Ändern',
    configure: 'Konfigurieren',
    accessDenied: 'Zugriff Verweigert',
    pleaseLoginToViewProfile: 'Bitte melden Sie sich an, um Ihr Profil anzuzeigen',
    orders: 'Bestellungen',
    products: 'Produkte',
    categories: 'Kategorien',
    users: 'Benutzer',
    analytics: 'Analysen',



    // Search
    searchPlaceholder: 'Finde dein Essen hier...',
    searchResults: 'Suchergebnisse',
    noResults: 'Keine Ergebnisse gefunden',
    searchFor: 'Suchen nach',
    searchProducts: 'Produkte suchen...',

    // Categories
    popular: 'Beliebt',
    allProducts: 'Alle Produkte',
    viewAll: 'Alle anzeigen',
    selectCategory: 'Kategorie auswählen',

    // Products
    product: 'Produkt',
    products: 'Produkte',
    addToCart: 'Zum Warenkorb hinzufügen',
    removeFromCart: 'Aus Warenkorb entfernen',
    outOfStock: 'Nicht auf Lager',
    inStock: 'Verfügbar',
    price: 'Preis',
    quantity: 'Menge',
    total: 'Gesamt',
    subtotal: 'Zwischensumme',
    tax: 'Steuer',
    shipping: 'Versand',
    discount: 'Rabatt',
    finalTotal: 'Endsumme',

    // Cart
    cartEmpty: 'Ihr Warenkorb ist leer',
    cartItems: 'Warenkorb Artikel',
    cartTotal: 'Warenkorb Gesamt',
    checkout: 'Zur Kasse',
    continueShopping: 'Weiter einkaufen',
    clearCart: 'Warenkorb leeren',
    updateCart: 'Warenkorb aktualisieren',
    cartCount: '{{count}} Artikel im Warenkorb',
    addToCart: 'Zum Warenkorb hinzufügen',
    quantity: 'Menge',
    each: 'jeweils',
    orderSummary: 'Bestellübersicht',
    item: 'Artikel',
    items: 'Artikel',
    filters: 'Filter',
    priceRange: 'Preisbereich',
    minPrice: 'Mindestpreis',
    maxPrice: 'Höchstpreis',
    sortBy: 'Sortieren nach',
    sortByName: 'Nach Name sortieren',
    sortByPriceAsc: 'Preis: Niedrig zu Hoch',
    sortByPriceDesc: 'Preis: Hoch zu Niedrig',
    sortByPopular: 'Beliebteste',
    gridView: 'Rasteransicht',
    listView: 'Listenansicht',
    toggleFilters: 'Filter umschalten',
    totalProducts: 'Gesamtprodukte',
    averagePrice: 'Durchschnittspreis',
    categories: 'Kategorien',
    featured: 'Empfohlen',

    // Orders
    order: 'Bestellung',
    orders: 'Bestellungen',
    orderNumber: 'Bestellung #{{number}}',
    orderDate: 'Bestelldatum',
    orderStatus: 'Bestellstatus',
    orderTotal: 'Bestellsumme',
    orderDetails: 'Bestelldetails',
    orderHistory: 'Bestellverlauf',
    pending: 'Ausstehend',
    confirmed: 'Bestätigt',
    preparing: 'In Vorbereitung',
    ready: 'Bereit',
    delivered: 'Geliefert',
    cancelled: 'Storniert',

    // Authentication
    email: 'E-Mail',
    password: 'Passwort',
    confirmPassword: 'Passwort bestätigen',
    firstName: 'Vorname',
    lastName: 'Nachname',
    phone: 'Telefon',
    address: 'Adresse',
    signIn: 'Anmelden',
    signUp: 'Registrieren',
    forgotPassword: 'Passwort vergessen?',
    resetPassword: 'Passwort zurücksetzen',
    createAccount: 'Konto erstellen',
    alreadyHaveAccount: 'Haben Sie bereits ein Konto?',
    dontHaveAccount: 'Haben Sie noch kein Konto?',
    loginSuccess: 'Anmeldung erfolgreich',
    loginError: 'Anmeldung fehlgeschlagen',
    logoutSuccess: 'Abmeldung erfolgreich',

    // Dashboard
    welcome: 'Willkommen',
    overview: 'Übersicht',
    recentOrders: 'Letzte Bestellungen',
    topProducts: 'Top Produkte',
    sales: 'Verkäufe',
    revenue: 'Umsatz',
    customers: 'Kunden',
    today: 'Heute',
    thisWeek: 'Diese Woche',
    thisMonth: 'Dieser Monat',
    thisYear: 'Dieses Jahr',

    // Notifications
    notification: 'Benachrichtigung',
    notifications: 'Benachrichtigungen',
    newOrder: 'Neue Bestellung',
    orderUpdated: 'Bestellung aktualisiert',
    orderCancelled: 'Bestellung storniert',
    orderDelivered: 'Bestellung geliefert',

    // Errors
    errorLoadingData: 'Fehler beim Laden der Daten. Bitte laden Sie die Seite neu.',
    errorSavingData: 'Fehler beim Speichern der Daten. Bitte versuchen Sie es erneut.',
    errorDeletingData: 'Fehler beim Löschen der Daten. Bitte versuchen Sie es erneut.',
    errorNetwork: 'Netzwerkfehler. Bitte überprüfen Sie Ihre Verbindung.',
    errorUnauthorized: 'Nicht autorisierter Zugriff.',
    errorForbidden: 'Zugriff verweigert.',
    errorNotFound: 'Seite nicht gefunden.',
    errorServer: 'Serverfehler. Bitte versuchen Sie es später erneut.',

    // Success Messages
    dataSaved: 'Daten erfolgreich gespeichert.',
    dataDeleted: 'Daten erfolgreich gelöscht.',
    orderPlaced: 'Bestellung erfolgreich aufgegeben.',
    cartUpdated: 'Warenkorb erfolgreich aktualisiert.',

    // Form Validation
    required: 'Dieses Feld ist erforderlich.',
    invalidEmail: 'Bitte geben Sie eine gültige E-Mail-Adresse ein.',
    passwordTooShort: 'Das Passwort muss mindestens 6 Zeichen lang sein.',
    passwordsDoNotMatch: 'Passwörter stimmen nicht überein.',
    invalidPhone: 'Bitte geben Sie eine gültige Telefonnummer ein.',

    // Time
    now: 'Jetzt',
    today: 'Heute',
    yesterday: 'Gestern',
    tomorrow: 'Morgen',
    minutes: 'Minuten',
    hours: 'Stunden',
    days: 'Tage',
    ago: 'vor',
    in: 'in',

    // Currency
    currency: '€',
    free: 'Kostenlos',

    // Language
    language: 'Sprache',
    english: 'Englisch',
    italian: 'Italienisch',
    german: 'Deutsch',
    spanish: 'Spanisch',

    // Theme
    theme: 'Design',
    light: 'Hell',
    dark: 'Dunkel',
    auto: 'Automatisch',

    // Actions
    viewDetails: 'Details anzeigen',
    editItem: 'Element bearbeiten',
    deleteItem: 'Element löschen',
    duplicateItem: 'Element duplizieren',
    exportData: 'Daten exportieren',
    importData: 'Daten importieren',
    download: 'Herunterladen',
    upload: 'Hochladen',
    print: 'Drucken',
    share: 'Teilen',
    copy: 'Kopieren',
    paste: 'Einfügen',
    cut: 'Ausschneiden',
    undo: 'Rückgängig',
    redo: 'Wiederholen',

    // Status
    active: 'Aktiv',
    inactive: 'Inaktiv',
    enabled: 'Aktiviert',
    disabled: 'Deaktiviert',
    online: 'Online',
    offline: 'Offline',
    available: 'Verfügbar',
    unavailable: 'Nicht verfügbar',

    // User Roles
    admin: 'Administrator',
    manager: 'Manager',
    waiter: 'Kellner',
    customer: 'Kunde',
    guest: 'Gast',

    // Waiter Dashboard
    newOrders: 'Neue Bestellungen',
    activeOrders: 'Aktive Bestellungen',
    completedOrders: 'Abgeschlossene Bestellungen',
    acceptOrder: 'Bestellung annehmen',
    rejectOrder: 'Bestellung ablehnen',
    markReady: 'Als bereit markieren',
    markDelivered: 'Als geliefert markieren',

    // Admin Dashboard
    adminDashboard: 'Admin Dashboard',
    systemOverview: 'Systemübersicht',
    userManagement: 'Benutzerverwaltung',
    productManagement: 'Produktverwaltung',
    orderManagement: 'Bestellverwaltung',
    systemSettings: 'Systemeinstellungen',
    reports: 'Berichte',
    backup: 'Backup',
    restore: 'Wiederherstellen',
    confirmDeleteUser: 'Sind Sie sicher, dass Sie diesen Benutzer löschen möchten?',
    confirmDeleteProduct: 'Sind Sie sicher, dass Sie dieses Produkt löschen möchten?',
    confirmDeleteCategory: 'Sind Sie sicher, dass Sie diese Kategorie löschen möchten?',
    confirmDeleteOrder: 'Sind Sie sicher, dass Sie diese Bestellung löschen möchten?',
    deleting: 'Löschen...',
    name: 'Name',
    description: 'Beschreibung',
    category: 'Kategorie',
    date: 'Datum',
    table: 'Tisch',
    tableNumber: 'Tisch {{number}}',

    // Login Page
    loginTitle: 'Anmelden',
    welcome: 'Willkommen',
    loginSubtitle: 'Melden Sie sich an, um auf das System zuzugreifen',
    username: 'Benutzername',
    password: 'Passwort',
    usernamePlaceholder: 'Geben Sie Ihren Benutzernamen ein',
    passwordPlaceholder: 'Geben Sie Ihr Passwort ein',
    signIn: 'Anmelden',
    signingIn: 'Anmeldung läuft...',
    demoCredentials: 'Demo-Anmeldedaten:',
    waiter: 'Kellner',
    manager: 'Manager',
    admin: 'Administrator',

    // Cart Page
    cart: 'Warenkorb',
    cartEmpty: 'Ihr Warenkorb ist leer',
    cartEmptyDescription: 'Fügen Sie einige leckere Produkte hinzu, um zu beginnen',
    viewMenu: 'Menü anzeigen',
    remove: 'Entfernen',
    clearCart: 'Warenkorb leeren',
    placeOrder: 'Bestellung aufgeben',
    orderCreatedSuccess: 'Bestellung erfolgreich erstellt',
    orderCreatedError: 'Fehler beim Erstellen der Bestellung. Bitte versuchen Sie es erneut.',
    total: 'Gesamt',

    // Waiter Dashboard
    waiterDashboard: 'Kellner Dashboard',
    welcomeUser: 'Willkommen, {{name}}',
    realTimeConnected: 'In Echtzeit verbunden',
    disconnected: 'Getrennt',
    logout: 'Abmelden',
    pending: 'Ausstehend',
    preparing: 'In Vorbereitung',
    ready: 'Bereit',
    delivered: 'Geliefert',
    cancelled: 'Storniert',
    orders: 'Bestellungen',
    noOrders: 'Keine Bestellungen',
    noOrdersDescription: 'Derzeit keine ausstehenden Bestellungen.',
    table: 'Tisch',
    tableNumber: 'Tisch {{number}}',
    products: 'Produkte',
    nextStatus: 'Nächster Status',
    updating: 'Aktualisierung...',
    markAsPreparing: 'Als Zubereitung markieren',
    markAsReady: 'Als Fertig markieren',
    markAsDelivered: 'Als Geliefert markieren',

    // Product Page
    product: 'Produkt',
    productNotFound: 'Produkt nicht gefunden',
    addToCart: 'Zum Warenkorb hinzufügen',
    quantity: 'Menge',

    // Not Found Page
    pageNotFound: 'Seite nicht gefunden',
    pageNotFoundDescription: 'Die gesuchte Seite existiert nicht oder wurde verschoben.',
    goBack: 'Zurück',

    // Menu Page
    menu: 'Menü',
    allCategories: 'Alle Kategorien',
    filterByCategory: 'Nach Kategorie filtern',
    discoverOurMenu: 'Entdecken Sie unser Menü',

    // Order Status
    orderStatus: 'Bestellstatus',
    orderNumber: 'Bestellung #{{number}}',
    orderDate: 'Bestelldatum',
    orderTotal: 'Bestellsumme',
    orderDetails: 'Bestelldetails',
    orderHistory: 'Bestellverlauf',

    // Connection Status
    connected: 'Verbunden',
    disconnected: 'Getrennt',
    connecting: 'Verbindung...',

    // Actions
    accept: 'Annehmen',
    reject: 'Ablehnen',
    markAsReady: 'Als bereit markieren',
    markAsDelivered: 'Als geliefert markieren',
    viewOrder: 'Bestellung anzeigen',
    editOrder: 'Bestellung bearbeiten',
    deleteOrder: 'Bestellung löschen',

    // Messages
    orderAccepted: 'Bestellung angenommen',
    orderRejected: 'Bestellung abgelehnt',
    orderReady: 'Bestellung bereit',
    orderDelivered: 'Bestellung geliefert',
    orderCancelled: 'Bestellung storniert',

    // Validation
    invalidUsername: 'Bitte geben Sie einen gültigen Benutzernamen ein',
    invalidPassword: 'Bitte geben Sie ein gültiges Passwort ein',
    loginFailed: 'Anmeldung fehlgeschlagen. Überprüfen Sie Ihre Anmeldedaten.',
    sessionExpired: 'Ihre Sitzung ist abgelaufen. Melden Sie sich erneut an.',

    // Time
    justNow: 'Gerade eben',
    aMinuteAgo: 'Vor einer Minute',
    minutesAgo: 'Vor {{count}} Minuten',
    anHourAgo: 'Vor einer Stunde',
    hoursAgo: 'Vor {{count}} Stunden',
    aDayAgo: 'Vor einem Tag',
    daysAgo: 'Vor {{count}} Tagen',

    // Currency and Numbers
    currencySymbol: '€',
    free: 'Kostenlos',
    price: 'Preis',
    subtotal: 'Zwischensumme',
    tax: 'Steuer',
    shipping: 'Versand',
    discount: 'Rabatt',
    finalTotal: 'Endsumme'
}; 