export default {
    // Common
    loading: 'Cargando...',
    error: 'Error',
    success: 'Éxito',
    cancel: 'Cancelar',
    save: 'Guardar',
    delete: 'Eliminar',
    edit: 'Editar',
    add: 'Agregar',
    back: 'Atrás',
    next: 'Siguiente',
    previous: 'Anterior',
    close: 'Cerrar',
    confirm: 'Confirmar',
    yes: 'Sí',
    no: 'No',
    all: 'Todos',
    view: 'Ver',
    search: 'Buscar',
    filter: 'Filtrar',
    sort: 'Ordenar',
    refresh: 'Actualizar',
    retry: 'Reintentar',
    reload: 'Recargar',

    // Navigation
    menu: 'Menú',
    cart: 'Carrito',
    login: 'Iniciar Sesión',
    logout: 'Cerrar Sesión',
    profile: 'Perfil',
    dashboard: 'Panel',
    settings: 'Configuración',
    userMenu: 'Menú de Usuario',
    user: 'Usuario',
    editProfile: 'Editar Perfil',
    save: 'Guardar',
    cancel: 'Cancelar',
    personalInformation: 'Información Personal',
    accountInformation: 'Información de Cuenta',
    accountSettings: 'Configuración de Cuenta',
    fullName: 'Nombre Completo',
    enterFullName: 'Ingresa tu nombre completo',
    enterUsername: 'Ingresa tu nombre de usuario',
    enterEmail: 'Ingresa tu dirección de email',
    notProvided: 'No proporcionado',
    memberSince: 'Miembro Desde',
    lastLogin: 'Último Inicio de Sesión',
    notAvailable: 'No disponible',
    changePassword: 'Cambiar Contraseña',
    changePasswordDescription: 'Actualiza tu contraseña de cuenta por seguridad',
    notificationSettings: 'Configuración de Notificaciones',
    notificationSettingsDescription: 'Configura cómo recibes las notificaciones',
    privacySettings: 'Configuración de Privacidad',
    privacySettingsDescription: 'Gestiona tus preferencias de privacidad',
    change: 'Cambiar',
    configure: 'Configurar',
    accessDenied: 'Acceso Denegado',
    pleaseLoginToViewProfile: 'Por favor inicia sesión para ver tu perfil',
    orders: 'Pedidos',
    products: 'Productos',
    categories: 'Categorías',
    users: 'Usuarios',
    analytics: 'Analíticas',



    // Search
    searchPlaceholder: 'Encuentra tu comida aquí...',
    searchResults: 'Resultados de búsqueda',
    noResults: 'No se encontraron resultados',
    searchFor: 'Buscar por',
    searchProducts: 'Buscar productos...',

    // Categories
    popular: 'Popular',
    allProducts: 'Todos los Productos',
    viewAll: 'Ver Todo',
    selectCategory: 'Seleccionar Categoría',

    // Products
    product: 'Producto',
    products: 'Productos',
    addToCart: 'Agregar al Carrito',
    removeFromCart: 'Quitar del Carrito',
    outOfStock: 'Agotado',
    inStock: 'Disponible',
    price: 'Precio',
    quantity: 'Cantidad',
    total: 'Total',
    subtotal: 'Subtotal',
    tax: 'Impuesto',
    shipping: 'Envío',
    discount: 'Descuento',
    finalTotal: 'Total Final',

    // Cart
    cartEmpty: 'Tu carrito está vacío',
    cartItems: 'Artículos del Carrito',
    cartTotal: 'Total del Carrito',
    checkout: 'Proceder al Pago',
    continueShopping: 'Continuar Comprando',
    clearCart: 'Vaciar Carrito',
    updateCart: 'Actualizar Carrito',
    cartCount: '{{count}} artículos en el carrito',
    addToCart: 'Añadir al Carrito',
    quantity: 'Cantidad',
    each: 'cada uno',
    orderSummary: 'Resumen del Pedido',
    item: 'artículo',
    items: 'artículos',
    filters: 'Filtros',
    priceRange: 'Rango de Precio',
    minPrice: 'Precio Mínimo',
    maxPrice: 'Precio Máximo',
    sortBy: 'Ordenar por',
    sortByName: 'Ordenar por Nombre',
    sortByPriceAsc: 'Precio: Menor a Mayor',
    sortByPriceDesc: 'Precio: Mayor a Menor',
    sortByPopular: 'Más Populares',
    gridView: 'Vista de Cuadrícula',
    listView: 'Vista de Lista',
    toggleFilters: 'Alternar Filtros',
    totalProducts: 'Total de Productos',
    averagePrice: 'Precio Promedio',
    categories: 'Categorías',
    featured: 'Destacados',

    // Orders
    order: 'Pedido',
    orders: 'Pedidos',
    orderNumber: 'Pedido #{{number}}',
    orderDate: 'Fecha del Pedido',
    orderStatus: 'Estado del Pedido',
    orderTotal: 'Total del Pedido',
    orderDetails: 'Detalles del Pedido',
    orderHistory: 'Historial de Pedidos',
    pending: 'Pendiente',
    confirmed: 'Confirmado',
    preparing: 'Preparando',
    ready: 'Listo',
    delivered: 'Entregado',
    cancelled: 'Cancelado',

    // Authentication
    email: 'Correo Electrónico',
    password: 'Contraseña',
    confirmPassword: 'Confirmar Contraseña',
    firstName: 'Nombre',
    lastName: 'Apellido',
    phone: 'Teléfono',
    address: 'Dirección',
    signIn: 'Iniciar Sesión',
    signUp: 'Registrarse',
    forgotPassword: '¿Olvidaste tu contraseña?',
    resetPassword: 'Restablecer Contraseña',
    createAccount: 'Crear Cuenta',
    alreadyHaveAccount: '¿Ya tienes una cuenta?',
    dontHaveAccount: '¿No tienes una cuenta?',
    loginSuccess: 'Inicio de sesión exitoso',
    loginError: 'Error al iniciar sesión',
    logoutSuccess: 'Cierre de sesión exitoso',

    // Dashboard
    welcome: 'Bienvenido',
    overview: 'Resumen',
    recentOrders: 'Pedidos Recientes',
    topProducts: 'Productos Principales',
    sales: 'Ventas',
    revenue: 'Ingresos',
    customers: 'Clientes',
    today: 'Hoy',
    thisWeek: 'Esta Semana',
    thisMonth: 'Este Mes',
    thisYear: 'Este Año',

    // Notifications
    notification: 'Notificación',
    notifications: 'Notificaciones',
    newOrder: 'Nuevo Pedido',
    orderUpdated: 'Pedido Actualizado',
    orderCancelled: 'Pedido Cancelado',
    orderDelivered: 'Pedido Entregado',

    // Errors
    errorLoadingData: 'Error al cargar los datos. Por favor, recarga la página.',
    errorSavingData: 'Error al guardar los datos. Por favor, inténtalo de nuevo.',
    errorDeletingData: 'Error al eliminar los datos. Por favor, inténtalo de nuevo.',
    errorNetwork: 'Error de red. Por favor, verifica tu conexión.',
    errorUnauthorized: 'Acceso no autorizado.',
    errorForbidden: 'Acceso prohibido.',
    errorNotFound: 'Página no encontrada.',
    errorServer: 'Error del servidor. Por favor, inténtalo más tarde.',

    // Success Messages
    dataSaved: 'Datos guardados exitosamente.',
    dataDeleted: 'Datos eliminados exitosamente.',
    orderPlaced: 'Pedido realizado exitosamente.',
    cartUpdated: 'Carrito actualizado exitosamente.',

    // Form Validation
    required: 'Este campo es obligatorio.',
    invalidEmail: 'Por favor, ingresa una dirección de correo válida.',
    passwordTooShort: 'La contraseña debe tener al menos 6 caracteres.',
    passwordsDoNotMatch: 'Las contraseñas no coinciden.',
    invalidPhone: 'Por favor, ingresa un número de teléfono válido.',

    // Time
    now: 'Ahora',
    today: 'Hoy',
    yesterday: 'Ayer',
    tomorrow: 'Mañana',
    minutes: 'minutos',
    hours: 'horas',
    days: 'días',
    ago: 'hace',
    in: 'en',

    // Currency
    currency: '€',
    free: 'Gratis',

    // Language
    language: 'Idioma',
    english: 'Inglés',
    italian: 'Italiano',
    german: 'Alemán',
    spanish: 'Español',

    // Theme
    theme: 'Tema',
    light: 'Claro',
    dark: 'Oscuro',
    auto: 'Automático',

    // Actions
    viewDetails: 'Ver Detalles',
    editItem: 'Editar Elemento',
    deleteItem: 'Eliminar Elemento',
    duplicateItem: 'Duplicar Elemento',
    exportData: 'Exportar Datos',
    importData: 'Importar Datos',
    download: 'Descargar',
    upload: 'Subir',
    print: 'Imprimir',
    share: 'Compartir',
    copy: 'Copiar',
    paste: 'Pegar',
    cut: 'Cortar',
    undo: 'Deshacer',
    redo: 'Rehacer',

    // Status
    active: 'Activo',
    inactive: 'Inactivo',
    enabled: 'Habilitado',
    disabled: 'Deshabilitado',
    online: 'En línea',
    offline: 'Desconectado',
    available: 'Disponible',
    unavailable: 'No disponible',

    // User Roles
    admin: 'Administrador',
    manager: 'Gerente',
    waiter: 'Camarero',
    customer: 'Cliente',
    guest: 'Invitado',

    // Waiter Dashboard
    newOrders: 'Nuevos Pedidos',
    activeOrders: 'Pedidos Activos',
    completedOrders: 'Pedidos Completados',
    acceptOrder: 'Aceptar Pedido',
    rejectOrder: 'Rechazar Pedido',
    markReady: 'Marcar como Listo',
    markDelivered: 'Marcar como Entregado',

    // Admin Dashboard
    adminDashboard: 'Panel de Administrador',
    systemOverview: 'Resumen del Sistema',
    userManagement: 'Gestión de Usuarios',
    productManagement: 'Gestión de Productos',
    orderManagement: 'Gestión de Pedidos',
    systemSettings: 'Configuración del Sistema',
    reports: 'Reportes',
    backup: 'Respaldo',
    restore: 'Restaurar',
    confirmDeleteUser: '¿Estás seguro de que quieres eliminar este usuario?',
    confirmDeleteProduct: '¿Estás seguro de que quieres eliminar este producto?',
    confirmDeleteCategory: '¿Estás seguro de que quieres eliminar esta categoría?',
    confirmDeleteOrder: '¿Estás seguro de que quieres eliminar este pedido?',
    deleting: 'Eliminando...',
    name: 'Nombre',
    description: 'Descripción',
    category: 'Categoría',
    date: 'Fecha',
    table: 'Mesa',
    tableNumber: 'Mesa {{number}}',

    // Login Page
    loginTitle: 'Iniciar Sesión',
    welcome: 'Bienvenido',
    loginSubtitle: 'Inicia sesión para acceder al sistema',
    username: 'Usuario',
    password: 'Contraseña',
    usernamePlaceholder: 'Ingresa tu usuario',
    passwordPlaceholder: 'Ingresa tu contraseña',
    signIn: 'Iniciar Sesión',
    signingIn: 'Iniciando sesión...',
    demoCredentials: 'Credenciales de Prueba:',
    waiter: 'Camarero',
    manager: 'Gerente',
    admin: 'Administrador',

    // Cart Page
    cart: 'Carrito',
    cartEmpty: 'Tu carrito está vacío',
    cartEmptyDescription: 'Agrega algunos productos deliciosos para comenzar',
    viewMenu: 'Ver Menú',
    remove: 'Eliminar',
    clearCart: 'Vaciar Carrito',
    placeOrder: 'Realizar Pedido',
    orderCreatedSuccess: 'Pedido creado exitosamente',
    orderCreatedError: 'Error al crear el pedido. Por favor, inténtalo de nuevo.',
    total: 'Total',

    // Waiter Dashboard
    waiterDashboard: 'Panel del Camarero',
    welcomeUser: 'Bienvenido, {{name}}',
    realTimeConnected: 'Conectado en tiempo real',
    disconnected: 'Desconectado',
    logout: 'Cerrar Sesión',
    pending: 'Pendiente',
    preparing: 'Preparando',
    ready: 'Listo',
    delivered: 'Entregado',
    cancelled: 'Cancelado',
    orders: 'Pedidos',
    noOrders: 'No hay pedidos',
    noOrdersDescription: 'No hay pedidos pendientes en este momento.',
    table: 'Mesa',
    tableNumber: 'Mesa {{number}}',
    products: 'productos',
    nextStatus: 'Siguiente Estado',
    updating: 'Actualizando...',
    markAsPreparing: 'Marcar como Preparando',

    // Product Page
    product: 'Producto',
    productNotFound: 'Producto no encontrado',
    addToCart: 'Agregar al Carrito',
    quantity: 'Cantidad',

    // Not Found Page
    pageNotFound: 'Página no encontrada',
    pageNotFoundDescription: 'La página que buscas no existe o ha sido movida.',
    goBack: 'Volver',

    // Menu Page
    menu: 'Menú',
    allCategories: 'Todas las Categorías',
    filterByCategory: 'Filtrar por categoría',
    discoverOurMenu: 'Descubre nuestro menú',

    // Order Status
    orderStatus: 'Estado del Pedido',
    orderNumber: 'Pedido #{{number}}',
    orderDate: 'Fecha del Pedido',
    orderTotal: 'Total del Pedido',
    orderDetails: 'Detalles del Pedido',
    orderHistory: 'Historial de Pedidos',

    // Connection Status
    connected: 'Conectado',
    disconnected: 'Desconectado',
    connecting: 'Conectando...',

    // Actions
    accept: 'Aceptar',
    reject: 'Rechazar',
    markAsReady: 'Marcar como Listo',
    markAsDelivered: 'Marcar como Entregado',
    viewOrder: 'Ver Pedido',
    editOrder: 'Editar Pedido',
    deleteOrder: 'Eliminar Pedido',

    // Messages
    orderAccepted: 'Pedido aceptado',
    orderRejected: 'Pedido rechazado',
    orderReady: 'Pedido listo',
    orderDelivered: 'Pedido entregado',
    orderCancelled: 'Pedido cancelado',

    // Validation
    invalidUsername: 'Por favor, ingresa un usuario válido',
    invalidPassword: 'Por favor, ingresa una contraseña válida',
    loginFailed: 'Error al iniciar sesión. Verifica tus credenciales.',
    sessionExpired: 'Tu sesión ha expirado. Por favor, inicia sesión de nuevo.',

    // Time
    justNow: 'Ahora mismo',
    aMinuteAgo: 'Hace un minuto',
    minutesAgo: 'Hace {{count}} minutos',
    anHourAgo: 'Hace una hora',
    hoursAgo: 'Hace {{count}} horas',
    aDayAgo: 'Hace un día',
    daysAgo: 'Hace {{count}} días',

    // Currency and Numbers
    currencySymbol: '€',
    free: 'Gratis',
    price: 'Precio',
    subtotal: 'Subtotal',
    tax: 'Impuesto',
    shipping: 'Envío',
    discount: 'Descuento',
    finalTotal: 'Total Final'
}; 